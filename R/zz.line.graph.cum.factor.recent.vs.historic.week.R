...
for(i in 1:nrow(factors.df)){
  ...
  plot.name = paste0("line.graph.cum.", as.character(i$name1), ".recent.vs.historic.week")
  ...
  temp = lapply(c("year.period.label2", as.character(i$factor)), as.symbol)
  
  table.to.plot = table.to.plot %>% group_by_(.dots = temp) %>% mutate(cum.freq = cumsum(Freq))
  ...
  p = p + geom_point(data = recent.table.to.plot,
                     aes_string(x="x.axis", y = "cum.freq", 
...
# line.graph.cum.all.recent.vs.historic.week
# line.graph.cum.sex.recent.vs.historic.week
# line.graph.cum.age.grp.recent.vs.historic.week
# line.graph.cum.request.recent.vs.historic.week
# line.graph.cum.spectype.recent.vs.historic.week
